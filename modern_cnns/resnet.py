...
class ResidualBasicBlock(nn.Module):
    ...
    Args:
        ...
        use_projection (bool): Whether to use a projection shortcut.
            When True, applies a 1x1 convolution to the input to match dimensions.
        stride (int): Stride for the first convolutional layer.
    """
    def __init__(self, out_channels: int, use_projection: bool, stride: int = 1):
        super().__init__()
...
class ResidualBottleneckBlock(nn.Module):
    ...
    """
    def __init__(self, mid_channels: int, stride: int = 1, expansion: int = 4):
        super().__init__()
        ...
        # Note that in bottleneck blocks, we always use projection for the shortcut,
        # since the number of channels is expanded by the expansion factor.
        self.shortcut = nn.Sequential(
            nn.LazyConv2d(out_channels, kernel_size=1, stride=stride, bias=False),
            nn.LazyBatchNorm2d(),
        )
...
class ResNetStage(nn.Module):
    ...
    def __init__(self, block_cls: Type[nn.Module], mid_channels: int, num_blocks: int,
        ...
        for i in range(num_blocks):
            use_projection = downsample and i == 0
            block_stride = stride if use_projection else 1
            if issubclass(block_cls, ResidualBottleneckBlock):
                b = block_cls(mid_channels, block_stride, expansion)
            elif issubclass(block_cls, ResidualBasicBlock):
                b = block_cls(mid_channels, use_projection, block_stride)
            else:
